#' A function to standardize photos for RICH economic games
#'
#' This function allows you to speed up photo standardization. Simply set a path to the main folder. The filenames of all raw photos should have same string length. These should be the IDs of the respondents. Then run the function. Photos can be croped, rotated, and resized to a constant aspect ratio. This function relies on 'imager'. Drag a box to crop. Eyeball the aspect ratio you want, then the code will enforce it. See details below.
#' @param 
...
#' @param 
#' id_range If a value is provided here OR in id_names, then not all photos will be processed. Use a vector of numbers here to indicate which IDs to process.  I.e., c(1:13) to run on only the first 13 photos.
#' @param 
#' id_names Or, just give the IDs themselves. I.e., c("XX1","XX2","XY1").
#' @param 
#' spin If spin is set to TRUE, then a two step process is opened. First select a spin, then crop. On the first pop-up, by clicking of the upper-left corner, the photo is not spun. If one clicks on the upper-right a 90 spin is done before opening the cropping window. Bottom-right, 180. Bottom-left, 270.
#' @export
#' @examples
#' \dontrun{
...
standardize_photos <- function(path=path,pattern=".JPG",start=1,stop=3,
 ...
 for( i in idset){
  ...
  if(spin==TRUE){
   loc<-grabPoint(img)                             # Check rotation
