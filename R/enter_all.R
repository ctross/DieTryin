enter_all <- function(path = "C:/Users/Mind Is Moving/Desktop/RICH/", pattern = ".jpg", start = 1, stop = 3, 
   ...
   res <- vector("list",length(img)-1)
    ...
    i <- 1
    X <- enter_data(path=path, pattern=pattern, start=start, stop=stop, seed=seed, n_frames=n_frames, n_rows=n_rows, n_cols=n_cols, 
   ...
   for(i in 1:(length(img)-1)){
     res[[i]] <- enter_data(path=path, pattern=pattern, start=start, stop=stop, seed=seed, n_frames=n_frames, n_rows=n_rows, n_cols=n_cols, 
                            lower_hue_threshold=lower_hue_threshold, upper_hue_threshold=upper_hue_threshold, colors=colors, thresh=thresh,
