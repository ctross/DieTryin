...
get_hue = function(path){
 ...
 y = imager::resize(img,round(size_out*asr,0),size_out)         

 y[1:round(dim(y)[1]*0.2,0),1:50,1,2:3] <-0
 y[1:round(dim(y)[1]*0.2,0),1:50,1,1] <-1

 y2 = imager::draw_text(y, 35, 15, "Exit", "black", opacity = 1, fsize = 30)
 y3 = imager::RGBtoHSL(y2)

 exit_path = 1
 safebreak = 1
 while(exit_path>0){

 loc = imager::grabPoint(y2)    
 ...
 y4 = imager::HSLtoRGB(y3)

 y2 = imager::draw_text(y4, round(dim(y)[1]*0.5,0) + 15, 15, paste0("Hue = ",hue_val), "black", opacity = 1, fsize = 30)
